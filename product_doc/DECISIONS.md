...
## GAME-2026-09-26 — 파이널 총점 동점 기준과 속도 설정 제거

- 상태: 사용자 직접 승인. 최종 총점 동점은 R5의 더 높은 경기 순위로 가르고, 카드 랭크를 별도 기준으로 다시 비교하지 않음.
- 결정: 최종 순위 정렬에서 총점 다음에 R5 경기 순위를 적용하고 추가 족보 비교를 제거. 두 기준도 모두 같으면 표의 안정적인 배열을 위해 플레이어 ID 순서만 사용하며, 공동 순위·랭킹 점수 분배 방식은 이번 요청에서 변경하지 않음.
- 설정: 환경설정의 미구현 속도 항목과 실제 싱글플레이 쇼다운 1×/2× 조절을 제거하고 기존 1× 재생 시간으로 고정. 온라인 서버 동기화 타이머는 변경하지 않음.
- 검증: `npm test` 464/464, `npm run test:workers` 13/13, `npm run lint`, `npm run build` 통과. Workers 테스트의 로컬 Wrangler 로그 권한 경고는 테스트 실패가 아님.
...
## BAL-005-SEED — 사전 승률 샘플링 시드의 카드 순서 불변화

- 상태: 사용자 승인 및 구현 완료. 확률 샘플링 결과의 재현성 개선이며 게임 규칙·밸런스 수치는 변경하지 않음.
