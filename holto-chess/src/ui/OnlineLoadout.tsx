...
export function OnlineLoadout({ me, disabled, onChange }: { me: PlayerView["me"]; disabled: boolean; onChange: (slots: (string | null)[]) => void }) {
  ...
  return <div className="loadout"><div className="loadout-games">{[1, 2].map((game) => <div className={`loadout-game game-${game}`} key={game}>
    ...
    <div className="loadout-sockets">{[0, 1].map((offset) => {
      ...
      return <div className="loadout-socket" key={index}>{card ? <CardView card={card} onClick={disabled ? undefined : () => onChange(toggleSlot(slots, owned, index))} footer={t("loadout.remove")} /> : <button className="playing-card loadout-empty" aria-label={t("loadout.autoSlotAria", { game, slot: offset + 1 })} disabled={disabled || owned.every((id) => slots.includes(id))} onClick={() => onChange(toggleSlot(slots, owned, index))}><span>＋</span></button>}</div>;
    })}</div>
  </div>)}</div>{owned.some((id) => !slots.includes(id)) && <div className="loadout-hand"><header><small>{t("loadout.unassigned")}</small></header><div className="card-row centered">{me.ownedCards.filter((card) => !slots.includes(card.id)).map((card) => <CardView key={card.id} card={card} />)}</div></div>}</div>;
}
