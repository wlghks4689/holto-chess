  ...
  return <CinematicGate key={`${credential?.roomId ?? "lobby"}:${displayView.me.playerId}`} matches={displayView.matches} profiles={displayView.players} viewerId={displayView.me.playerId} presentation={displayView.presentation} clock={serverClock}><main className={displayView.phase !== "LOBBY" ? "game-arena" : "arena-lobby"}><nav><button className="brand brand-home" type="button" onClick={onHome} aria-label="PORENA 메인 화면으로 이동"><span>P</span><div><b>PORENA</b><small>ONLINE · TACTICAL POKER AUTOBATTLER</small></div></button>{displayView.phase !== "LOBBY" ? <RoundProgress round={displayView.round} prep={null} /> : <span />}<div className="survivors"><small>CONNECTION</small><b className={credential ? `conn-${status.toLowerCase()}` : "conn-lobby"}>{credential ? statusLabel : "로비"}</b></div></nav>
    <div className="page-shell" id="top">
      ...
      <>
        ...
        {displayView.phase === "GAME_RESULT" && <><FinalResultsPanel view={displayView} /><section className="panel rematch-panel"><span className="eyebrow">PLAY AGAIN</span><h2>같은 방에서 새 게임</h2><p>현재 참가자와 방 코드를 그대로 유지합니다. 모두 선택하면 즉시 새 게임을 시작합니다.</p><button className="primary" disabled={disabled || meReadyForRematch || rematchHumans.length < 2} onClick={() => send({ type: "REMATCH_READY" })}>{meReadyForRematch ? `새 게임 대기 중 · ${rematchReady}/${rematchHumans.length}` : "같은 방에서 새 게임하기"}</button></section></>}
