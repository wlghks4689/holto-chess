...
export function OnlineApp({ onHome }: { onHome: () => void }) {
  ...
  return <CinematicGate key={`${credential?.roomId ?? "lobby"}:${displayView.me.playerId}`} matches={displayView.matches} profiles={displayView.players} viewerId={displayView.me.playerId} presentation={displayView.presentation} clock={serverClock}><main className={displayView.phase !== "LOBBY" ? "game-arena" : "arena-lobby"}><nav><button className="brand brand-home" type="button" onClick={onHome} aria-label="PORENA 메인 화면으로 이동"><span>P</span><div><b>PORENA</b><small>ONLINE · TACTICAL POKER AUTOBATTLER</small></div></button>{displayView.phase !== "LOBBY" ? <RoundProgress round={displayView.round} prep={null} /> : <span />}<div className="survivors"><small>CONNECTION</small><b className={credential ? `conn-${status.toLowerCase()}` : "conn-lobby"}>{credential ? statusLabel : "로비"}</b></div></nav>
    <div className="page-shell" id="top">
      ...
      <>
        ...
        <header className="round-header"><div><span className="round-number">ROUND 0{displayView.round}</span><h1>{titles[displayView.round]}</h1></div><div className="phase-badge"><b>{phases[displayView.phase] ?? displayView.phase}</b></div></header>
        <OnlineScoreboard view={displayView} />
        ...
        {displayView.phase === "SHOP" && displayView.me.alive && <><section className="shop-layout"><div className="inventory panel"><header><h2>{isSpectatingPlayer ? `${observedName} 카드` : "내 카드"} <em>{displayView.me.ownedCards.length} / {displayView.me.handLimit}</em></h2><div className="stat-block"><strong>{displayView.me.stackBB}<i>BB</i></strong></div></header><div className="card-row owned-row">{displayView.me.ownedCards.map((card) => <CardView key={card.id} card={card} onClick={!interactionDisabled && !displayView.me.committed && canSell ? () => send({ type: "SELL_CARD", cardId: card.id }) : undefined} footer={isSpectatingPlayer ? undefined : canSell ? "판매" : "판매 불가"} />)}<EmptyHandSlots count={displayView.me.ownedCards.length} limit={displayView.me.handLimit} /></div><p className="hint">{isSpectatingPlayer ? "관전 화면에서는 카드를 조작할 수 없습니다." : canSell ? `판매 환급 ${displayView.me.sellPercent}% · 판매 후 구매 횟수는 복구되지 않습니다.` : "남은 구매 횟수로 필수 보유 장수를 복구할 수 없어 더 이상 판매할 수 없습니다."}</p></div><div className="market panel"><header><div><span className="eyebrow">TWO CARD DEAL</span><h2>카드 마켓</h2></div><span className="purchase-count">구매 {displayView.me.purchases} / {displayView.me.purchaseLimit}</span></header><div className="card-row market-row">{displayView.me.shopCards.map(({ card, price }, index) => <ShopCard key={card.id} dealIndex={index} card={card} price={price} locked={displayView.me.lockedShopCardIds?.includes(card.id) ?? false} disabled={interactionDisabled || displayView.me.committed} onBuy={() => send({ type: "BUY_CARD", cardId: card.id })} onLock={() => send({ type: "LOCK_SHOP", cardId: card.id })} />)}</div><div className="market-actions"><button className="secondary" disabled={interactionDisabled || displayView.me.committed || allShopCardsLocked || displayView.me.rerollsUsed >= displayView.me.rerollLimit || displayView.me.stackBB < displayView.me.rerollCost} onClick={() => send({ type: "REROLL" })}>{pending === "REROLL" ? "REFRESHING…" : `리롤 ${displayView.me.rerollCost}BB`} · {displayView.me.rerollsUsed} / {displayView.me.rerollLimit}</button><span className="hint">{allShopCardsLocked ? "모든 카드가 잠겨 리롤할 수 없습니다." : "카드별 잠금 3BB · 해제 무료"}</span></div></div></section>
          {(displayView.round === 2 || displayView.round === 3) && <Selection key={`${displayView.round}:${displayView.me.playerId}:${displayView.me.ownedCards.map((c) => c.id).join()}`} view={displayView} send={send} disabled={interactionDisabled || displayView.me.committed} />}
          <div className="action-bar shop-ready-bar">
            ...
            <button className="primary" disabled={interactionDisabled || displayView.me.committed || displayView.me.ownedCards.length !== displayView.me.handLimit || (displayView.round === 2 && displayView.me.selectedCardIds.length !== 2)} onClick={() => send({ type: "END_SHOP_PHASE" })}>{displayView.me.committed ? "준비 완료 ✓" : "준비 완료 · 구성 확정"}</button></div></>}
        {displayView.phase === "GAME_RESULT" && <FinalResultsPanel view={displayView} />}
        <RoundResults round={displayView.round} rows={displayView.roundSummary ?? []} viewerId={displayView.me.playerId}>{(displayView.roundHistory ?? displayView.matches).map((m) => <OnlineMatch key={m.id} match={m} view={displayView} />)}</RoundResults>
        ...
        {!view.me.alive && !view.players.find((p) => p.playerId === view.me.playerId)?.departed && <section className="panel eliminated-panel"><h2>탈락했습니다</h2><p className="hint">생존 플레이어를 선택해 해당 플레이어의 화면을 읽기 전용으로 관전할 수 있습니다.</p><div className="room-controls">{spectating ? <button className="secondary" onClick={() => { setSpectating(false); setSpectatedPlayerId(null); }}>관전 화면 닫기</button> : <button className="secondary" disabled={!view.spectatorViews?.length} onClick={() => { setSpectating(true); setSpectatedPlayerId(view.spectatorViews?.[0]?.playerId ?? null); }}>다른 플레이어 관전</button>}<button className="secondary" disabled={disabled} onClick={leaveRoom}>방 나가기</button></div></section>}
