  ...
  return <CinematicGate key={credential?.roomId ?? "lobby"} matches={view?.matches ?? []} profiles={view?.players ?? []} viewerId={view?.me.playerId ?? ""} presentation={view?.presentation} clock={serverClock}><main className={view && view.phase !== "LOBBY" ? "game-arena" : "arena-lobby"}><nav><a className="brand" href="#top"><span>P</span><div><b>PORENA</b><small>ONLINE · TACTICAL POKER AUTOBATTLER</small></div></a>{view && view.phase !== "LOBBY" ? <RoundProgress round={view.round} prep={null} /> : <span />}<div className="survivors"><small>CONNECTION</small><b className={credential ? `conn-${status.toLowerCase()}` : "conn-lobby"}>{credential ? statusLabel : "로비"}</b></div></nav>
    <div className="page-shell" id="top">
      ...
      {view && <>
        ...
        {!["LOBBY", "SHOP", "AUGMENT", "GAME_RESULT"].includes(view.phase) && <div className="action-bar phase-ready-bar"><div className="phase-wait-copy"><b>{phases[view.phase]}</b><p>{view.phase === "NEXT_ROUND" ? "이번 라운드가 끝났습니다. 확인하면 다음 라운드 상점으로 이동합니다." : view.phase === "SHOWDOWN_PRIMARY" || view.phase === "SHOWDOWN_SECONDARY" ? "카드 구성이 확정되었습니다. 모두 확인하면 쇼다운을 시작합니다." : "결과를 확인해 주세요. 모두 확인하면 다음 단계로 이동합니다."}</p></div>{secondsLeft !== null && <div className="action-countdown" role="timer" aria-label={`${phases[view.phase]} 남은 시간 ${secondsLeft}초`}><small>남은 시간</small><strong>{formatCountdown(secondsLeft)}</strong></div>}<button className="primary" disabled={disabled || view.players.find((p) => p.playerId === view.me.playerId)?.ready || (!view.me.alive && view.players.some((p) => p.human && p.alive))} onClick={() => send({ type: "READY" })}>{!waitingOnMe ? "확인 완료 · 다른 플레이어 대기" : view.phase === "NEXT_ROUND" ? "다음 라운드 상점으로" : view.phase === "SHOWDOWN_PRIMARY" || view.phase === "SHOWDOWN_SECONDARY" ? "쇼다운 시작 준비" : "확인 · 다음 단계"}</button></div>}
      </>}
    </div>
  </main></CinematicGate>;
}
