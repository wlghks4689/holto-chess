...
const phases: Record<string, string> = { LOBBY: "입장 대기", SHOP: "상점", SHOWDOWN_PRIMARY: "VS · 전장 준비", GROUP_ASSIGNMENT: "그룹 배정", SHOWDOWN_SECONDARY: "2차전 VS · 전장 준비", ROUND_RESULT: "라운드 결과", AUGMENT: "증강 선택", NEXT_ROUND: "다음 라운드", GAME_RESULT: "최종 결과" };
...
export function OnlineApp({ onHome }: { onHome: () => void }) {
  ...
  return <CinematicGate key={`${credential?.roomId ?? "lobby"}:${displayView.me.playerId}`} matches={displayView.matches} profiles={displayView.players} viewerId={displayView.me.playerId} presentation={displayView.presentation} clock={serverClock}><main className={displayView.phase !== "LOBBY" ? "game-arena" : "arena-lobby"}><nav><button className="brand brand-home" type="button" onClick={onHome} aria-label="PORENA 메인 화면으로 이동"><span><img src="/assets/brand/porena-mark.webp" alt="" width="38" height="38" /></span><div><b>PORENA</b><small>ONLINE · TACTICAL POKER AUTOBATTLER</small></div></button>{displayView.phase !== "LOBBY" ? <RoundProgress round={displayView.round} prep={null} /> : <span />}<div className="nav-status"><div className="survivors"><small>CONNECTION</small><b className={credential ? `conn-${status.toLowerCase()}` : "conn-lobby"}>{credential ? statusLabel : "로비"}</b></div><button type="button" className="secondary nav-exit" disabled={!!pending} onClick={() => setExiting(true)}>나가기</button></div></nav>
    ...
    <div className={`page-shell ${displayView.phase === "SHOP" ? "shop-page" : ""}`} id="top">
      ...
      <>
        ...
        <header className="round-header"><div><span className="round-number">ROUND 0{displayView.round}</span><h1>{displayView.phase === "GAME_RESULT" ? "FINAL STANDINGS" : titles[displayView.round]}</h1></div><div className="phase-badge"><b>{phases[displayView.phase] ?? displayView.phase}</b></div></header>
        ...
        {["SHOWDOWN_PRIMARY", "SHOWDOWN_SECONDARY"].includes(displayView.phase) && <div className="action-bar phase-ready-bar phase-auto-bar"><div className="phase-wait-copy"><b>{phases[displayView.phase]}</b><p>다음 상대와 전장을 동기화하고 있습니다. 준비가 끝나면 자동으로 시작합니다.</p></div>{secondsLeft !== null && <PhaseTimer className="action-countdown" seconds={secondsLeft} ariaLabel={`${phases[displayView.phase]} 남은 시간 ${secondsLeft}초`} />}<strong>자동 진행</strong></div>}
