  ...
  return <div className="round-guide-backdrop" role="presentation">
    <section className="round-guide" role="dialog" aria-modal="true" aria-labelledby="round-guide-title">
      ...
      <footer><p>{onPreviewRound ? "각 라운드를 선택해 규칙을 확인하세요." : typeof secondsLeft === "number"
        ...
        : "라운드가 시작될 때 한 번 표시됩니다."}</p><button className="primary" type="button" onClick={onClose}>{onPreviewRound ? "시작 화면으로" : `이해했습니다 · ROUND ${round} 시작`} <span>→</span></button></footer>
    </section>
  </div>;
}
