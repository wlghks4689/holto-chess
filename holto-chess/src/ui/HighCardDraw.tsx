...
export function HighCardDrawResult({ draw, name, survival }: {
  ...
  return <section className="high-card-result" aria-label="하이카드 드로우 결과">
    <h3>하이카드 드로우</h3>
    <div className="high-card-players">{draw.draws.map((entry) => <div key={entry.playerId} className={entry.playerId === draw.winnerId ? "draw-winner" : ""}>
      <b>{name(entry.playerId)}</b><strong>{rankLabel(entry.rank)}</strong>
      <span>{entry.playerId === draw.winnerId ? survival ? "생존" : "승리" : survival ? "탈락" : "패배"}</span>
    </div>)}</div>
    <p>족보 무승부 · {name(draw.winnerId)} 하이카드 드로우 {survival ? "생존" : "승리"}</p>
  </section>;
}

export function HighCardDrawNotice({ survival, seconds }: { survival: boolean; seconds: number }) {
  return <div className="high-card-backdrop"><section className="high-card-notice" role="alertdialog" aria-modal="true" aria-labelledby="high-card-title" aria-describedby="high-card-rules">
    ...
    <div id="high-card-rules"><p>동률인 플레이어가 서로 다른 숫자 카드 1장씩을 뽑습니다.</p>
      <strong>{survival ? "가장 높은 카드 1명만 생존합니다. 나머지 플레이어는 탈락합니다." : "가장 높은 카드를 뽑은 플레이어가 승리합니다."}</strong>
