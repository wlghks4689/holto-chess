...
export function OpenDraftPanel({ view, send, disabled, seconds }: {
  ...
  return <section className="open-draft panel" aria-label={`R${view.round} 공개 드래프트`}>
    <header><small className="draft-kicker">ROUND {view.round} · {ordering ? "DRAFT ORDER" : "OPEN DRAFT"}</small><div className="draft-clock" role="timer"><small>선택 제한</small><strong>{seconds ?? 20}</strong><span>초</span></div><h2>{ordering ? "낮은 승점부터 선택합니다" : "공유 아레나에서 한 장을 선택하세요"}</h2></header>
    ...
    {!ordering && <><div className="draft-arena">{draft.cards.map(({ card, price, claimedBy }) => <div key={card.id} className={`draft-offer ${claimedBy ? "claimed" : ""}`}>
      <CardView card={card} footer={`${price} BB`} onClick={myTurn && !disabled && !claimedBy && view.me.stackBB >= price ? () => send({ type: "DRAFT_PICK", cardId: card.id }) : undefined} />
...
export function RunLoadoutPanel({ view, send, disabled, seconds }: {
  ...
  return <section className="panel run-loadout"><small>RUN LOADOUT · 세 장 모두 사용</small><h2>대표 카드와 두 RUN을 설계하세요</h2>
    ...
    <div className="action-bar"><span>남은 시간 <b>{seconds ?? 60}초</b> · 미완성 배치는 자동 완성</span><button className="primary" disabled={disabled || ready} onClick={() => send({ type: "LOCK_RUN_LOADOUT" })}>{ready ? "LOADOUT LOCKED" : "배치 확정 · 준비 완료"}</button></div>
  </section>;
}
